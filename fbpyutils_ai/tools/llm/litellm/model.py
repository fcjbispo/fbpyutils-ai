...
def list_models(api_base_url: str, api_key: str, **kwargs: Any) -> List[Dict[str, Any]]:
    try:
        selected_models = []
        llm_provider = next(
            iter([p for p in LLM_PROVIDERS.values() if p["base_url"] == api_base_url]),
            None,
        )
        if llm_provider:
            ...
            llm_models = _model_prices_and_context_window.get_model_prices_and_context_window_by_provider(
                provider
            )
...
def get_model_details(
    ...
    **kwargs: Any,
    ...
    try:
        ...
        llm_models = [
            m for m in list_models(api_base_url, api_key) if m["id"] == model_id
        ]
        ...
        try:
            model_prices_and_context_window = ModelPricesAndContextWindow().get_model_prices_and_context_window_by_provider(
                provider
            )
        except Exception as e:
        ...
        response_data.update(model_prices_and_context_window.get(model_id, {}))
        ...
        if introspection:
            ...
            llm_model_supported_parameters = (
            ...
            llm_model_details["supported_ai_parameters"] = (
                llm_model_supported_parameters
            )
