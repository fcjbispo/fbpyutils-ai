...
async def test_searxng_tool_search_success(mock_http_client):
    ...
    mock_http_client.sync_request.return_value = {"results": [{"title": "Test Result", "url": "http://example.com"}]}
...
async def test_searxng_tool_async_search_success(mock_http_client):
    ...
    tool = SearXNGTool(base_url="http://test_url")
    tool.http_client = mock_http_client
    async def mock_async_request(*args, **kwargs): return {"results": [{"title": "Test Result", "url": "http://example.com"}]}
...
def test_searxng_utils_simplify_results():
    ...
    results = [{"url": "http://example.com", "title": "Test Result", "content": "Test content", "score": 0.5, "publishedDate": "2024-01-01", "other": "extra"}]
...
def test_searxng_utils_convert_to_dataframe():
    ...
    import pandas as pd
    results = [{"url": "http://example.com", "title": "Test Result", "content": "Test content", "score": 0.5, "publishedDate": "2024-01-01", "other_info": {"extra": "info"}}]
