...
@pytest.mark.asyncio
async def test_async_request_retry_on_http_error(mock_async_client, caplog):
    """Test async_request retries on HTTPStatusError."""
    mock_request_obj = httpx.Request("GET", "https://api.example.com/retry_error")
    error_response_500 = httpx.Response(
        status_code=500, text="Internal Server Error", request=mock_request_obj
    )
    error_response_400 = httpx.Response(
        status_code=400, text="Bad Request", request=mock_request_obj
    )
    success_response = httpx.Response(
        status_code=200, json={"status": "ok"}, request=mock_request_obj
    )

    # Simulate 2 failures (500, 500) followed by success (200)
    mock_async_client.get.side_effect = [
        httpx.HTTPStatusError("Server Error", request=mock_request_obj, response=error_response_500),
        httpx.HTTPStatusError("Server Error", request=mock_request_obj, response=error_response_500),
        success_response,
    ]
    ...
    async with HTTPClient(base_url="https://api.example.com") as client:
        ...
        response = await client.async_request("GET", "/retry_error", stop=stop_after_attempt(3)) # Use stop parameter

        assert isinstance(response, httpx.Response)
        assert response.status_code == 200
        assert response.json() == {"status": "ok"}
        # Verify that the mock was called 3 times (2 retries + 1 success)
        assert mock_async_client.get.call_count == 3
        assert "Attempt 1 failed" in caplog.text # Check for tenacity logs
        assert "Attempt 2 failed" in caplog.text
        assert "Attempt 3 succeeded" in caplog.text


@pytest.mark.asyncio
async def test_async_request_retry_on_network_error(mock_async_client, caplog):
    """Test async_request retries on NetworkError."""
    mock_request_obj = httpx.Request("GET", "https://api.example.com/retry_network")
    success_response = httpx.Response(
        status_code=200, json={"status": "ok"}, request=mock_request_obj
    )

    # Simulate 2 network errors followed by success
    mock_async_client.get.side_effect = [
        httpx.NetworkError("Connection refused", request=mock_request_obj),
        httpx.NetworkError("Connection timed out", request=mock_request_obj),
        success_response,
    ]
    ...
    async with HTTPClient(base_url="https://api.example.com") as client:
        ...
        response = await client.async_request("GET", "/retry_network", stop=stop_after_attempt(3)) # Use stop parameter

        assert isinstance(response, httpx.Response)
        assert response.status_code == 200
        assert response.json() == {"status": "ok"}
        # Verify that the mock was called 3 times
        assert mock_async_client.get.call_count == 3
        assert "Attempt 1 failed" in caplog.text # Check for tenacity logs
        assert "Attempt 2 failed" in caplog.text
        assert "Attempt 3 succeeded" in caplog.text
...
def test_sync_request_retry_on_http_error(mock_sync_client, caplog):
    """Test sync_request retries on HTTPStatusError."""
    mock_request_obj = httpx.Request("GET", "https://api.example.com/retry_error_sync")
    error_response_500 = httpx.Response(
        status_code=500, text="Internal Server Error", request=mock_request_obj
    )
    success_response = httpx.Response(
    ...
    with HTTPClient(base_url="https://api.example.com") as client:
        ...
        response = client.sync_request("GET", "/retry_error_sync", stop=stop_after_attempt(3)) # Use stop parameter

        assert isinstance(response, httpx.Response)
        assert response.status_code == 200
        assert response.json() == {"status": "ok"}
        # Verify that the mock was called 3 times
        assert mock_sync_client.get.call_count == 3
        assert "Attempt 1 failed" in caplog.text # Check for tenacity logs
        assert "Attempt 2 failed" in caplog.text
        assert "Attempt 3 succeeded" in caplog.text


def test_sync_request_retry_on_network_error(mock_sync_client, caplog):
    """Test sync_request retries on NetworkError."""
    mock_request_obj = httpx.Request("GET", "https://api.example.com/retry_network_sync")
    success_response = httpx.Response(
        status_code=200, json={"status": "ok"}, request=mock_request_obj
    )
    ...
    with HTTPClient(base_url="https://api.example.com") as client:
        ...
        response = client.sync_request("GET", "/retry_network_sync", stop=stop_after_attempt(3)) # Use stop parameter
