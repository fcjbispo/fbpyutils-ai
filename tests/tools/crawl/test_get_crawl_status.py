...
# Test cases for get_crawl_status
@patch('fbpyutils_ai.tools.crawl.HTTPClient')
def test_get_crawl_status_success(mock_http_client):
    # Arrange
    mock_client_instance = mock_http_client.return_value
    mock_response_data = {
        "status": "completed",
        "current": 10,
        "current_url": "http://example.com/page10",
    ...
    # Act
    status = tool.get_crawl_status(job_id)
...
@patch('fbpyutils_ai.tools.crawl.HTTPClient')
def test_get_crawl_status_job_not_found(mock_http_client):
    # Arrange
    mock_client_instance = mock_http_client.return_value
...
@patch('fbpyutils_ai.tools.crawl.HTTPClient')
def test_get_crawl_status_api_error(mock_http_client):
    # Arrange
    mock_client_instance = mock_http_client.return_value
    mock_client_instance.sync_request.side_effect = httpx.HTTPStatusError(
...
@patch('fbpyutils_ai.tools.crawl.HTTPClient')
def test_get_crawl_status_connection_error(mock_http_client):
    # Arrange
    mock_client_instance = mock_http_client.return_value
